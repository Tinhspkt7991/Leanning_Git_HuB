...
int main(void)
{
  ...
  /*--------------------------------------------------------------------------*/
  AES_TEST_ASSERT(((INOUTSEL0 & 0xFFFF) == 0x0000)
    && ((INOUTSEL1 & 0x0007) == 0x0000) && ((INOUTSEL2 & 0x0007) == 0x0000));

  /*--------------------------------------------------------------------------*/
  /* Step 3:                                                                  */
  /*  Invoke Port_SetPinDirection() API  to set the  pin to Direction.        */
  ...
  /* Check valid register. */
  /*--------------------------------------------------------------------------*/
   /* PORT_PIN_OUT, PORT_PIN_POSITIVE_LOGIC, PORT_PIN_OUTDT, PORT_PIN_LEVEL_LOW*/   
   AES_TEST_ASSERT(((INOUTSEL0 & 0xFFFF) == 0xFFFF) 
   && ((OUTDT0 & 0xFFFF) == 0x0000));
   
   AES_TEST_ASSERT(((INOUTSEL1 & 0x0007) == 0x0007));
   AES_TEST_ASSERT(((INOUTSEL2 & 0x0007) == 0x0007));
  /*--------------------------------------------------------------------------*/
  /* Step 5:                                                                  */
  /*  Invoke  Port_RefreshPortDirection() API  to set the  pin to Direction.  */

  /*--------------------------------------------------------------------------*/
   Port_RefreshPortDirection();
  /* Check the PortGroup0, PortGroup1, PortGroup2 registers */
  AES_TEST_ASSERT(((INOUTSEL0 & 0xFFFF) == 0xFFFF)
    && ((INOUTSEL1 & 0x0007) == 0x0000) && ((INOUTSEL2 & 0x0007) == 0x0000));
  AES_Report_Result();
}
