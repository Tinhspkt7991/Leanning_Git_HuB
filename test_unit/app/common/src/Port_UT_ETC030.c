...
/*----------------------------------------------------------------------------*/
  ...
  AES_TEST_ASSERT((E_OK == Dem_TestNoError()) && (E_OK == Det_TestNoError()));
  ...
  /*                                                                          */
  /*--------------------------------------------------------------------------*/
  RCar_RfsoFaultControl(PORT_CONFIGURATION_CHECK_ENABLE_RFSO, RFSO_FAULT_RESET);
  
  Port_ConfigurationRegisterCheck(PORT_TRANSIENT_CHECK_OFF);

  /* Check LenCheckResult = PORT_E_CONFIGURATION_CHECK_PERMANENT_FAILURE      */
  AES_TEST_ASSERT(E_OK == 
    Dem_TestLastReportErrorStatus(PORT_E_CONFIGURATION_CHECK_PERMANENT_FAILURE, 
      DEM_EVENT_STATUS_FAILED));

  AES_TEST_ASSERT((RFSO0 & 0x0006) == 0x0006);

  AES_TEST_ASSERT(E_OK == Det_TestNoError());
